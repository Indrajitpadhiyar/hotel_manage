import React from 'react';
import { useNavigate } from 'react-router-dom';
import StarRating from '../components/StarRating';
import { roomData } from '../assets/roomData';
import location from '../assets/location.webp';

const AllRooms = () => {
  const navigate = useNavigate();

  return (
    <div className='flex flex-col lg:flex-row items-start justify-between pt-28 px-4 md:px-16 lg:px-24 xl:px-32'>
      
      <div>
        
        {/* Heading */}
        <div className='flex flex-col items-start text-left'>
          <h1 className='font-playfair text-4xl md:text-[40px]'>
            Hotel Rooms
          </h1>
          <p className='text-sm md:text-base text-gray-500/90 mt-2'>
            Take advantage of our limited-time offers and special packages to enhance your stay.
          </p>
        </div>

        {/* Rooms List */}
        {roomData.map((room) => (
          <div
            key={room._id}
            className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-20 last:border-0'
          >
            
            {/* Image */}
            <img
              onClick={() => {
                navigate(`/rooms/${room._id}`);
                window.scrollTo(0, 0);
              }}
              src={room.image}
              alt='hotel-img'
              className='h-60 w-full md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'
            />

            {/* Content */}
            <div className='md:w-1/2 flex flex-col gap-2'>
              
              <p className='text-gray-500'>{room.city}</p>

              <p
                onClick={() => {
                  navigate(`/rooms/${room._id}`);
                  window.scrollTo(0, 0);
                }}
                className='text-gray-800 text-3xl font-playfair cursor-pointer'
              >
                {room.name}
              </p>

              <div className='flex items-center'>
                <StarRating />
                <p className='ml-2'>200+ reviews</p>
              </div>

              {/* Location */}
              <div className='flex items-center gap-1 text-gray-500 mt-2 text-sm'>
                <img src={location} alt='location' className='w-5 h-5' />
                <span>{room.hotel?.address || "Address not available"}</span>
              </div>

              {/* Amenities */}
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities?.map((item, index) => (
                  <div
                    key={index}
                    className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'
                  >
                    <p className='text-xs'>{item}</p>
                  </div>
                ))}
              </div>
              <p className='text-2xl font-bold text-gray-800'>
                ${room.price.toFixed(2)}/night
              </p>
            </div>
          </div>
        ))}

      </div>
      <div className='BG-WHITE W-80 BORDER BORDER-GRAY-300 TEXT-GRAY-600 MAX-LG:MB-8 MIN-LG:MT-16'>

      </div>
    </div>
  );
};

export default AllRooms;